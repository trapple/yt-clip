...
| E2E | 拡張がロードされ、YouTube 上で UI が注入され、IN/OUT の範囲が確定すること | Playwright の永続コンテキストで拡張をロード。MVP ではスモークのみ |

E2E の自動化範囲は **IN/OUT の範囲確定まで**とする。録画そのものは自動化できない。`chrome.tabCapture` は「拡張がそのタブに対して呼び出されたこと」を要求し、実際にはツールバーのアイコンをクリックして popup を開く操作がそれにあたる。Playwright は popup を URL 直接ナビゲーションで開くためこの呼び出しが発生せず、`getStreamId` が `Extension has not been invoked for the current page` で失敗する。ブラウザの仕様上の制約であり、実装側で回避すべきものではない。

同様に X への添付検証もログイン済みアカウントを要し、自動化すると認証情報の管理が必要になる上に X の自動化ポリシー上も望ましくない。

したがって **録画・添付はリリース前の手動確認項目**として扱う。
