...
|---|---|
| バー | 中身の根 `#yt-clip-bar` がバーの窓の中にある (**文書につながっているかは見ない**: 窓が差す先ごと一時的に外れた枠の中にあっても、中身があれば出す側に数える。`document.getElementById` で見ると、外れた枠の中のバーを隠した後に sync が body へ退避しても、`mount` の早抜けが出し直しを呼ばず隠れたまま残る) |
...
- **差し直し**: `mount()` (DOM 変化のたびに呼ばれる) で、2 つの枠が差す先の中にあるか (`anchor.contains(slot)`) を確かめ、
  ...
  (`replaceChildren` で外されるだけ)。**差す先そのものが無い** (SPA 遷移の途中) なら使えない枠として扱う
  (C2.1 の退避。枠の根は外す)。差す先はあるが幅 0 なら C2.1 の退避
- **動画ページ以外では、ページに何も差さない** (spa-inject の不具合修正で追加)。YouTube は動画ページからホームへ SPA で
  移っても `#below` / `#secondary-inner` (隠れた `ytd-watch-flexy` の中) を残すので、差す先の有無ではなく URL
  (`currentVideoId() !== null`) で決める: 動画ページ以外では 2 つの枠の根を外し、3 つの窓も body から外す (隠すだけに
  しない)。枠の中身 (tabs / active) と窓の位置はメモリに残り、動画ページへ戻った `mount()` が差し直す。バーの組み立て・
  枠の差し込み・窓の出し入れは動画ページでだけ行う
