...
**並べ替えない。重なりもマージしない。** 当初は「時系列に自動ソートし、
重なりは 1 つに繋ぐ」と決めていたが、実機で使って 2 つの問題が出たので撤回した。

**1. 区間の中で「＋ 区間を追加」を押すと無反応になる。** 既存区間 `1:23〜2:00`
の中の `1:30` で押すと、新しい区間 `1:30〜1:45` は完全に含まれるのでマージ後の
結果が元と同じになる。押したのに何も起きない。

**2. 同じ場面を 2 回使えない。** マージは「情報を失わない」と考えていたが、
これは誤りだった。重なる区間を別々に持ちたい場面 (同じカットを繰り返す等) を
潰している。

**代わりに拾った順をそのまま出力順にする。** 得られるものは 3 つある。

- 押せば必ず区間が増える
- 「オチを先に見せる」ような並べ方ができる
- **index が動かない**ので、選択の同定に仕掛けが要らない (§2.4)

**代償は録画中の巻き戻しシーク。** 拾った順に辿るので、後ろの区間から前の
区間へ戻ることがある。バッファの再読み込みが入って待ち時間が伸びうるが、
繋ぎ目の品質そのものは `waitForFreshFrame` (§4.3) が守る。**新しいフレームが
描かれるまで `resume()` しない**ので、前の場面が混入することはない。

`assertValidSegments` がするのは検証だけである。

- `startSec` が負、`endSec <= startSec`、`NaN` / `Infinity` は throw
- 空配列は許す (`idle` へ落とすのは呼び出し側の判断)

### 2.4 index は動かないので ID を振らない

並べ替えもマージもしないので、**一度できた区間の位置は変わらない**。ハンドルを
ドラッグして隣を追い越しても、一覧での位置はそのままである。

**区間に ID を振る必要がない。** `ClipRange` をそのまま使える。

index が動く唯一の経路は削除で、これは算術で追える。

| 削除した index | 選択中の index | 選択はどうなるか |
|---|---|---|
| 選択より前 | — | 1 つ手前へずらす |
| 選択と同じ | — | 同じ位置に来た区間 (無ければ末尾) |
| 選択より後 | — | そのまま |

**開始秒で引き直す方式は採れない。** 重なる区間を許した以上、ある秒を含む
区間は複数ありうる。どちらを指すか決められない。
...
### 9.2 巻き戻しシークの検証結果 (2026-09-23)

§2.3 で自動ソートをやめたことにより、**後ろの区間から前の区間へ戻る録画**が
起きるようになった。当初の設計が避けていた経路なので、実機で確かめた。

**繋ぎ目に前の場面の混入はなく、待ち時間も実用の範囲だった。** バッファの
再読み込みが入りやすいという懸念はあったが、`waitForFreshFrame` が新しい
フレームを待つため、品質は前進シークと変わらない。

**これでこの spec の未検証の前提はすべて潰れた。**
